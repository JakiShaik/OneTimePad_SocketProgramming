...
int main(int argc, char *argv[]) {
  ...
  if(pid == 0) {
    ...
    //printf("string is %s\n",string);
    if(string[0] == 'D') {
      fprintf(stderr,"Can not connect to otp_enc_d daemon\n");
      exit(1);
      //error("This is not Otp_dec daemon");
    }
    //printf("lline 47\n");
    ...
    //printf("SERVER: I recieved this plaintext from the client: \"%s\"\n",str[0]);
    ...
    for(i=0;i<strlen(str[0]);i++) {
      ...
      if(pascii == 0) {fprintf(stderr,"Bad file"); exit(1);}
